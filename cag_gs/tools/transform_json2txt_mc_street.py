...
if __name__ == "__main__":
    ...
    parser.add_argument("--destiny_path", type=str, required=True)
    ...
    sparse_path = os.path.join(destiny_path, 'sparse')
    if not os.path.exists(sparse_path):
        os.mkdir(sparse_path)
    input_path = os.path.join(destiny_path, 'images')
    ...
    with open(images_path, 'w') as f:
        ...
        for idx, frame in enumerate(contents['frames']):
            ...
            if not os.path.exists(os.path.join(source_path, 'input', file_path)):
                ...
                os.system("cp {} {}".format(os.path.join(data_path, 'rgb', data_file_path), os.path.join(destiny_path, 'images', file_path)))
