...
from internal.dataparsers.dataparser import DataParserOutputs, ImageSet, PointCloud
...
class PartitionableScene:
    ...
    def run(self, output_path: str):
        ...
        for cell_idx in range(len(partition_coords)):
            part_objs = self.plot_cell(ax, cell_idx, partition_coords, campos, camera_assign)
            fig.savefig(osp.join(output_path, "figures", "part_{}.png".format(cell_idx)), dpi=300)
    ...
    def compute_camera_visibility(
        ...
        for cam_idx, camera in tqdm(enumerate(cameras)):
            ...
            for part_idx in range(len(partition_coords)):
                cam_vis[part_idx, cam_idx] = anchor_weights[is_in_bboxes[i]].sum() / total_weights
        return cam_vis
    ...
    def save_partitions(
        ...
        for cell_idx, (part_id, part_xy, part_size) in enumerate(tqdm(partition_coords, desc="Saving partitions")):
            cell_dir = osp.join(output_path, "partitions", "part_{:03d}".format(cell_idx))
